...
class BookShop{
	...
	public:
		...
		/* Use the title **MergedShop** of the new bookshop	*/
            ...
            merged.size = this->size + b.size;
            merged.count = this->count + b.count;

            strcpy(merged.name, "MergedShop");

            merged.books = new Book[merged.size];

            for(int i = 0; i < this->count; i++) {
                merged.books[i] = this->books[i];
            }

            for(int i = 0; i < b.count; i++) {
                merged.books[this->count + i] = b.books[i];
            }
